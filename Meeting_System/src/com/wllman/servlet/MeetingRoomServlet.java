...
public class MeetingRoomServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<Meetingroom> list =dao.selectMeetingrooms();
		request.setAttribute("list", list);
		int role = (Integer)session.getAttribute("role");
		//if (role == 1) {
		request.getRequestDispatcher("showmeetingroom.jsp").forward(request, response);
		//}else {
			//request.getRequestDispatcher("usershowmeetingroom.jsp").forward(request, response);
		//}
